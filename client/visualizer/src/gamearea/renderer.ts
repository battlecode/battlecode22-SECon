...
export default class Renderer {
    ...
    constructor(readonly canvas: HTMLCanvasElement, readonly imgs: AllImages, private conf: config.Config, readonly metadata: Metadata,
        ...
        readonly onMouseover: (x: number, y: number, xrel: number, yrel: number, wall: number, uranium: number) => void) {
    ...
    private renderResources(world: GameWorld) {
        ...
        for (let i = 0; i < width; i++) for (let j = 0; j < height; j++) {
            ...
            if (map.uraniumVals[idxVal] > 0) {
                let size = sigmoid(map.uraniumVals[idxVal] / 50) * .94
                if (!this.conf.doingRotate) this.ctx.drawImage(uraniumImg, cx + (1 - size) / 2, cy + (1 - size) / 2, scale * size, scale * size)
                else this.ctx.drawImage(uraniumImg, cy + (1 - size) / 2, cx + (1 - size) / 2, scale * size, scale * size)
    ...
    private setMouseoverEvent(world: GameWorld) {
        ...
        if (this.hoverPos) {
            ...
            this.onMouseover(x, y, xrel, yrel, world.mapStats.walls[idx], world.mapStats.uraniumVals[idx])
        }
