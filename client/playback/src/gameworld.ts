...
export type MapStats = {
    ...
    spawnLocations: { color: string, x: number, y: number }[]
    symmetry: number
...
export default class GameWorld {
    ...
    loadFromMatchHeader(header: schema.MatchHeader) {
        ...
        this.mapStats.spawnLocations = []
        for (let i = 0; i < map.spawnLocation().teamIDsLength(); i++) {
            let loc: { color: string, x: number, y: number } = { color: undefined, x: 0, y: 0 }
            loc.color = map.spawnLocation().teamIDs(i) == 1 ? "red" : "blue"

            loc.x = map.spawnLocation().locs().xs(i)!
            loc.y = map.spawnLocation().locs().ys(i)!
            this.mapStats.spawnLocations.push(loc)
        }
    ...
    processDelta(delta: schema.Round) { // Change to reflect current game
        ...
        if (delta.actionsLength() > 0) {
            ...
            for (let i = 0; i < delta.actionsLength(); i++) {
                ...
                const body = (robotID != -1 && this.bodies.index(robotID) != -1) ? this.bodies.lookup(robotID) : null
